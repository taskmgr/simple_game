...
void myprog() {
	initWindow("lalala", 300, 300, 300, 300);
	beginPaint();                                   //必须要有的开始绘制函数 
	ACL_Image mpa;
	loadImage("C:\\Users\\rainielove\\Desktop\\tmp.gif",&mpa);
	putImageScale(&mpa, 0, 0,100,100);
	endPaint();
	clearDevice();
	beginPaint();
	putImageScale(&mpa, 100, 100, 100, 100);
	//setPenWidth(1);                                 //定义画笔的宽度为1个像素 
	//setPenColor(RED);                               //定义画笔的颜色为红色 
	//setPenStyle(PEN_STYLE_DASHDOT);                 //定义画笔的线条形状为点线型.-.- 
	//setBrushColor(GREEN);                           //定义刷子的颜色为绿色 
	//setBrushStyle(BRUSH_STYLE_DIAGCROSS);           //定义刷子的形状为X 
	//for (int i = 1; i < 15; i++) {
	//	roundrect(20 * i, 20 * i, 280, 280, i, i);
	//}
	endPaint();                                     //必须要有的结束绘制函数 
